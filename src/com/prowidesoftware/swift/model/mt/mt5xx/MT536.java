...
// end qualifiers constants	
	private static transient final java.util.logging.Logger log = java.util.logging.Logger.getLogger(MT536.class.getName());
	...
	/**
	 * Iterates through block4 fields and return the first one whose name matches 95C, 
	 * or <code>null</code> if none is found.<br />
	 * The first occurrence of field 95C at MT536 is expected to be the only one.
	 * 
	 * @return a Field95C object or <code>null</code> if the field is not found
	 * @see SwiftTagListBlock#getTagByName(String)
	 * @throws IllegalStateException if SwiftMessage object is not initialized
	 */
	public Field95C getField95C() {
		if (getSwiftMessage() == null) {
			throw new IllegalStateException("SwiftMessage was not initialized");
		}
		if (getSwiftMessage().getBlock4() == null) {
			...
			final Tag t = getSwiftMessage().getBlock4().getTagByName("95C");
			if (t == null) {
				log.fine("field 95C not found");
				return null;
			} else {
				return new Field95C(t.getValue());
			}
		}
	}
...
// BaseSequenceCodeGenerator [seq=B1a2]
	...
	*/
	...
	public List<SequenceB1a2> getSequenceB1a2List() {
		final List<SwiftTagListBlock> blocks = getSwiftMessage().getBlock4().getSubBlocks(SequenceB1a2.START_END_16RS);
		if (blocks != null && !blocks.isEmpty()) {
			final List<SequenceB1a2> result = new ArrayList<SequenceB1a2>(blocks.size());
			for (final SwiftTagListBlock b:blocks) {
				result.add(new SequenceB1a2(b));
			}
			return result; 
		}
		// TODO if is is mandatory issue a warning log
...
// BaseSequenceCodeGenerator [seq=B1a2A]
	...
	*/
	...
	public List<SequenceB1a2A> getSequenceB1a2AList() {
		final List<SwiftTagListBlock> blocks = getSwiftMessage().getBlock4().getSubBlocks(SequenceB1a2A.START_END_16RS);
		if (blocks != null && !blocks.isEmpty()) {
			final List<SequenceB1a2A> result = new ArrayList<SequenceB1a2A>(blocks.size());
			for (final SwiftTagListBlock b:blocks) {
				result.add(new SequenceB1a2A(b));
			}
			return result; 
		}
		// TODO if is is mandatory issue a warning log
